...
const MOCK_ATTENDEES = [
  ...
  },
  {
    ...
    name: "Biscuit",
    ownerName: "Tanya P.",
    image: "https://images.unsplash.com/photo-1537151625747-768eb6cf92b2?w=200&h=200&fit=crop",
  ...
  },
  {
    ...
    name: "Luna",
    ownerName: "Kate S.",
    image: "https://images.unsplash.com/photo-1561037404-61cd46aa615b?w=200&h=200&fit=crop",
    breed: "Shiba Inu",
    size: "MD",
    slot: "13:00 – 15:00",
...
function AttendeeCard({
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#f1f5f9] px-4 py-4 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          <p className="text-[15px] font-bold text-[#1e293b] truncate">{attendee.name}</p>
          <span
            className={clsx(
      ...
      <div className="flex flex-col items-end gap-1 shrink-0">
        <div className="flex items-center gap-1 bg-[#f8fafc] rounded-lg px-2 py-1">
          <span className="material-symbols-outlined text-[#94a3b8]" style={{ fontSize: 12 }}>
            schedule
          </span>
          <span className="text-[11px] font-semibold text-[#64748b]">{attendee.slot}</span>
        </div>
      </div>
    </div>
  );
      ...
      <div className="w-full max-w-lg px-4 py-5 flex flex-col gap-5">
        ...
        <div className="bg-white rounded-2xl border border-[#f1f5f9] px-5 py-4 flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-[rgba(226,207,183,0.3)] flex items-center justify-center shrink-0">
            <span className="material-symbols-outlined text-[#1e293b]">calendar_month</span>
          </div>
          <div>
            <p className="text-[15px] font-bold text-[#1e293b]">{formattedDate}</p>
            <p className="text-[12px] text-[#94a3b8] mt-0.5">
              {isToday ? "Today · " : ""}{MOCK_ATTENDEES.length} attendees across {slots.length} slots
            </p>
          </div>
        </div>
          ...
          return (
            <div key={slot} className="flex flex-col gap-2">
              <div className="flex items-center gap-2 px-1">
                <span className="material-symbols-outlined text-[#e2cfb7]" style={{ fontSize: 15 }}>
                  schedule
                </span>
        ...
        {MOCK_ATTENDEES.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            <div className="w-16 h-16 rounded-full bg-[rgba(226,207,183,0.2)] flex items-center justify-center">
              <span className="material-symbols-outlined text-[#e2cfb7]" style={{ fontSize: 32 }}>
                pets
              </span>
            </div>
            <p className="text-[15px] font-bold text-[#1e293b]">No attendees yet</p>

...
type Props = {
  ...
  minAdvanceDays?: number; // minimum days ahead required to book (default 0)
};
...
export default function DateRangeCalendar({
  ...
  const [month, setMonth] = useState(
    dayjs().add(minAdvanceDays, "day").startOf("month"),
  );
  const [selectingEnd, setSelectingEnd] = useState(false);
  const [selectedSlotId, setSelectedSlotId] = useState<string | null>(null);
  const changeSlot = (id: string | null) => {
      ...
      <div className="bg-white rounded-[14px] border border-[rgba(226,207,183,0.4)] overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-[rgba(226,207,183,0.3)]">
          <button
            ...
            disabled={
              !readOnly && month.isSame(dayjs().startOf("month"), "month")
            }
        ...
        <div className="grid grid-cols-7 gap-0.5 px-2 pb-3">
          {(() => {
            ...
            return cells.map((day, i) => {
              ...
              const minDate = today.add(minAdvanceDays, "day");
              const isPast = day.isBefore(minDate);
              const isClosed =
                !readOnly &&
                !!availableWeekdays &&
                !availableWeekdays.has((day.day() + 6) % 7);
              ...
              return (
                <button
                  ...
                  className={clsx(
                    ...
                    readOnly &&
                      hasAttendees &&
                      "hover:bg-[rgba(226,207,183,0.15)] cursor-pointer",
        ...
        {!readOnly &&
          (() => {
            const slotsForDay =
              slots && startDay
                ? slots.filter(
                    (s) => s.weekday === (dayjs(startDay).day() + 6) % 7,
                  )
                : [];
            const selectedSlot =
              slotsForDay.find((s) => s._id === selectedSlotId) ?? null;
            return (
              <div className="border-t border-[rgba(226,207,183,0.3)] px-4 py-3 flex flex-col gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-2">
                      {slotsForDay.map((slot) => {
                        const isFull =
                          !!slot.maxDogs &&
                          (slot.attendeesId?.length ?? 0) >= slot.maxDogs;
                        return (
                          <button
                            key={slot._id}
                            type="button"
                            ...
                            onClick={() => {
                              ...
                              onEndChange(
                                `${isSingleDate ? day : endDay || day}T${slot.endTime}`,
                              );
                            }}
                            className={clsx(
                          ...
                          >
                            {slot.label && (
                              <span className="block text-[11px] font-bold mb-0.5 opacity-70">
                                {slot.label}
                              </span>
                            )}
                            ...
                            {isFull && (
                              <span className="ml-1 text-[10px]">Full</span>
                            )}
                          </button>
                        );
                      })}
                ...
                <div
                  className={clsx(
                    "grid gap-3",
                    isSingleDate ? "grid-cols-1" : "grid-cols-2",
                  )}
                >
                  <div>
                    <p className="text-[10px] font-bold text-[#94a3b8] uppercase tracking-wide mb-1.5">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (selectedSlot) {
                          if (val < selectedSlot.startTime)
                            val = selectedSlot.startTime;
                          if (val > selectedSlot.endTime)
                            val = selectedSlot.endTime;
                        }
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          onEndChange(
                            `${endDay || startDay || dayjs().format("YYYY-MM-DD")}T${val}`,
                          );
                        }}
                        className="w-full h-10 rounded-lg border border-[rgba(226,207,183,0.4)] bg-white px-3 text-[13px] text-[#1e293b] outline-none focus:border-[#e1cfb7]"
                      />
                ...
                {startDay && (isSingleDate || endDay) && (
                  <div className="flex items-center justify-between text-[12px] font-semibold text-[#64748b] bg-[#f8fafc] rounded-lg px-3 py-2">
                    <span>
                      {dayjs(startDay).format("D MMM")} · {startTime}
                    </span>
                    {!isSingleDate && (
                      <>
                        <span
                        ...
                        <span>
                          {dayjs(endDay).format("D MMM")} · {endTime}
                        </span>
                      </>
                    )}
                  </div>
